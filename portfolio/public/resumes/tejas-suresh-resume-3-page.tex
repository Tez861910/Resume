...
\begin{itemize}[leftmargin=0.15in, label={}]
  \small{\item{
    ...
    \textbf{Mobile \& Cross-Platform}{: Flutter, Dart, Rust, Flutter Rust Bridge, local-first architecture} \\
...
\resumeSubHeadingListStart
  ...
  \resumeSubheading
    ...
    \resumeItemListStart
      ...
      \resumeItem{Delivered an examination management system that automated assessments and reduced manual overhead by about \textbf{50\%}.}
...
\resumeSubHeadingListStart
  \resumeProjectHeading
    ...
    \resumeItemListStart
      \resumeItem{Local-first toolbox with \textbf{30+ utilities} spanning PDF, image, QR, OCR, storage, audio, and developer workflows.}
      \resumeItem{Designed the split architecture between Flutter screens and a Rust core connected through Flutter Rust Bridge.}
      \resumeItem{Structured the product by tool domains so broad utility coverage stays maintainable instead of collapsing into one large feature blob.}
      \resumeItem{Kept packaging and release paths in scope across Android, Windows, and Apple-platform targets.}
    \resumeItemListEnd
    \vspace{-8pt}
  ...
  \resumeProjectHeading
    ...
    \resumeItemListStart
      \resumeItem{Built the main portfolio as a fast recruiter-facing route with deeper case studies, then added an optional cockpit mode for the immersive version of the same story.}
      \resumeItem{Structured the project data model so private product work can be explained with more honesty and detail than a bullet-only resume allows.}
